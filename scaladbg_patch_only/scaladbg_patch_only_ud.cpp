...
static void build_matrices(int size, int numkmers, std::vector<std::vector<int> >&send_recv,
    ...
    std::queue<int> kvals);
static void get_assembly_info(string read_file, string long_read_file, AssemblyInfo &assembly_info);
...
{
    double readTime, recvTime, scaffTime;
    double sendTime,totalTime;
    int rank, size;
    queue <int> kvals;
    OptionsDescription desc;
    desc.AddOption("kfile", "k", option.k_values_file, "name of file that contains the kvalues, 1 on each line in ascending order");
    ...
    if(DEBUG == 1) printf("My rank is %i and size is %i\n",rank,size);

    try{
        desc.Parse(argc, argv);
        if (option.k_values_file == "")
            throw logic_error("Must specify k_values_file");
    }
    catch (exception &e){
        cerr << e.what() << endl;
        cerr << "ScalaDBG - Scalable Genomic Assembly through Parrallel de Bruijn Graph Construction for Multiple K-mers" <<endl;
        cerr << "Usage: scaladbg --kfile kfile.txt -o output_dir" << endl;
    ...
    //read the option.k_values_file into a queue
    ifstream in(option.k_values_file.c_str(),ios::in);
    int value;
    int numkmers = 0;
    while( in >> value){
        kvals.push(value);
    ...
    vector< vector <int> > communication;
    build_matrices(size, numkmers, send_recv, communication, k_val_work, kvals);
    ...
    for(unsigned int i = 0; i < send_recv[rank].size(); i++){
        if(send_recv[rank][i] == SEND){
            if(communication[rank][i] != -1){
                ...
                sendTime += MPI_Wtime();
                if(TIMING==1)printf("Sending from %i to %i took %f seconds\n",rank, sendingTo, sendTime);               
            }
            if(!k_val_work[rank].empty()){
                double build_time = -MPI_Wtime();
                kmer_size = k_val_work[rank][0];
                ...
                get_assembly_info("", FormatString("%d_input.fa",kmer_size), assembly_info);
                
                HashGraph hash_graph(kmer_size);
                newBuildHashGraph(kmer_size,hash_graph);
                if((i+1) < send_recv[rank].size() && send_recv[rank][i+1] == RECV){
                    Assemble(hash_graph);
                }
                else{
                    char filename [33];
                ...
                if(TIMING==1){
                    printf("Rank %i size of long sequences is %lu\n", rank, assembly_info.long_reads.size());
                    printf("Build time for kmer_size %i is %f seconds\n",kmer_size,build_time);
                }
            }
        }
            ...
            if(TIMING==1){
                ...
                printf("AlignReads for kmer_size %i took %f seconds\n",kmer_size,align_time);
                printf("CorrectReads for kmer_size %i took %f seconds\n",kmer_size,correct_time);
                printf("LocalAssembly for kmer_size %i took %f seconds\n",kmer_size,local_time);
                printf("Patching or UpdateHashGraph for kmer_size %i took %f seconds\n",kmer_size,patch_time);
            }
            if(((i+1) < send_recv[rank].size() && send_recv[rank][i+1] == RECV)){//if we are receiving next we will patch, need contigs from this one to patch :)
                ...
                if(TIMING==1)printf("Assemble for kmer_size %i took %f seconds\n",kmer_size,assemble_time);   
            }
    ...
    if(rank==MASTER){
        if(DEBUG==1) printf("rank %i is the master finishing, kmer_size is %i\n",rank,kmer_size);
        readTime = -MPI_Wtime();
        kmer_size = kvals.back();
        ...
        readTime += MPI_Wtime();
        if(TIMING==1)printf("ReadSequence/writer time for kmer_size %i took %f seconds\n",kmer_size,readTime);
        scaffTime = -MPI_Wtime();
        // option.maxk = kmer_size;
        // Scaffold(kmer_size, option.min_contig);
        // scaffTime += MPI_Wtime();
...
static void newBuildHashGraph(int kmer_size, HashGraph& hash_graph)
{
    ...
    BuildKmerFile(assembly_info, kmer_size, 0, option.prefix_length, option.kmer_file()+buffer);
